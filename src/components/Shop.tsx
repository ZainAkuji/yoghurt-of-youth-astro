...
function computeTotals(
  ...
  const deliveryFee = merchTotal === 0 ? 0 : 0.01;
                ...
                return (
                  <div key={f.id} className="grid grid-rows-[auto,auto] gap-px">
                    ...
                    <div className={cn("relative px-2 py-2 flex items-center justify-center gap-2", !isMix && !isTaster && (f as any).bg)}>
                      ...
                      <button
                        onClick={() => {
                          if (f.id === ids.PLN) { storeSetQty(ids.PLN, qty(ids.PLN) + 7); bumpDisplay(f.id, 1); trackAddToCart("PLN", 7 * 0.1, 7); return; }
                          ...
                          if (f.id === "TASTER") { incPreset("TASTER"); trackAddToCart("Taster", 0.1 + 2.9 * 3, 4); return; }
